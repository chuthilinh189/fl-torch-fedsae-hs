...
class ClientFedGH:
    def __init__(self, args, client_idx, train_data_loader, val_data_loader, test_data_loader):
        ...
        self.lambda_re = getattr(self.args, "lambda_re", 0.0)
    ...
    def get_predictor_parameters(self):
        return copy.deepcopy(self.net.predictor.state_dict())

    def set_predictor_parameters(self, predictor_state):
        self.net.predictor.load_state_dict(copy.deepcopy(predictor_state), strict=True)
    ...
    def set_recent_metric(self, recent_re, recent_ce, recent_train_loss, recent_val_loss):
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            for input, label in self.val_data_loader:
                ...
                label_bin = (label != 0).long()
                logits, recon = self.net(input)
                total_loss, _, _, _ = self.calculate_loss(input, label_bin, logits, recon)
                list_loss.append(float(total_loss.item()))

        avg_loss = float(np.mean(list_loss)) if list_loss else 0.0
        self.recent_val_loss = avg_loss
        return avg_loss
    ...
    def test_with_logits(self, verbose=False):
        ...
        roc = float(roc_auc_score(labels_bin, prob_attack)) if len(set(labels_bin)) > 1 else 0.0
    ...
    def collect_prototypes(self):
        ...
        with torch.no_grad():
            for input, label in self.train_data_loader:
                input, label = input.to(self.device), label.to(self.device)
                label_bin = (label != 0).long()
                latent = self.net.encoder(input)
                for i in range(latent.size(0)):
                    lbl = int(label_bin[i].item())
                    vec = latent[i].detach().cpu()
                    if lbl not in proto_sums:
                        proto_sums[lbl] = vec.clone()
                        proto_counts[lbl] = 1
                    else:
                        proto_sums[lbl] += vec
                        proto_counts[lbl] += 1
        proto_stats = {}
        for lbl, s in proto_sums.items():
            proto_stats[lbl] = (s.numpy(), proto_counts[lbl])
        return proto_stats
